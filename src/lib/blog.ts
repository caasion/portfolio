...
export async function getPostData(id: string): Promise<PostData> {
    ...
    return { id, ...(data as { title: string; date: string; description: string; authors: string[]; tags: string[];  }), coverImage, contentHTML };
}
...
export function getSortedBlogData(): PostIndexData[] {
    ...
    const allBlogData = dirNames
        .map(slug => {
            ...
            return { id: slug, ...(data as { title: string; date: string; description: string; authors: string[]; tags: string[]; }), coverImage}
        })
